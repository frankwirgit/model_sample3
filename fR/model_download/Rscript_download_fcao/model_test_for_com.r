...
#run the glm model to create the model reg_com
...
tmp <- computePredCI2(patient_org, feature_est, coe.com, cov.com)

#6.2 using the imputed binary rsult
tmb <- computePredCI2(patient_org, data.frame(feature_est_bin), coe.com, cov.com)
...
feature_lwr[d]=a
tmr <- computePredCI3(patient_org, feature_est, feature_upr, feature_lwr, coe.com, cov.com)

#6.4 using the imputed range plus confident interval
tmf <- computePredCI4(patient_org, feature_est, feature_range, coe.com, cov.com)
...
td <- computePredCI2(patient_org, patient_org, coe.com, cov.com)
td$diff=0
td$diff_pct=0
td$true_value="***"
td$next_question=NA
...
z1 <- aggregate(ewmp[,2:5], by=list(as.numeric(as.character(ewmp$x))), mean)
colnames(z1)[1]="x"
z2 <- aggregate(ewmb[,2:5], by=list(as.numeric(as.character(ewmb$x))), mean)
colnames(z2)[1]="x"
z3 <- aggregate(ewmr[,2:5], by=list(as.numeric(as.character(ewmr$x))), mean)
colnames(z3)[1]="x"
z4 <- aggregate(ewmf[,2:5], by=list(as.numeric(as.character(ewmf$x))), mean)
colnames(z4)[1]="x"

par(mfrow=c(1,2))
matplot(z1$x, cbind(z1$y, z2$y, z4$y), axes=F, col=c("black","grey","brown"), pch=19, 
main="Random order, Average CI comparison - \nfixed P(black) | binary(grey) | imputed range(brown)")
axis(2)
axis(1,at=seq(1,10,by=1))
abline(h=0)

matplot(z1$x, cbind(z1$yy, z2$yy, z4$yy), axes=F, col=c("black","grey","brown"), pch=19, 
main="Random order, Average varies(%) comparison \nfixed P(black) | binary(grey) | imputed range(brown)")
axis(2)
axis(1,at=seq(1,10,by=1))
abline(h=0)

matplot(z1$x, cbind(z1$z, z2$z), axes=F, col=c("black","pink"), pch=19, 
main="Random order, Average error distance comparison - \nfixed P(black) | binary(pink)")
axis(2)
axis(1,at=seq(1,10,by=1))
abline(h=0)

matplot(z1$x, cbind(z1$zz, z2$zz), axes=F, col=c("black","pink"), pch=19, 
main="Random order, Average error distance(%) comparison \nfixed P(black) | binary(pink)")
axis(2)
axis(1,at=seq(1,10,by=1))
abline(h=0)
